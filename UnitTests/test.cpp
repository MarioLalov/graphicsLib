...
TEST_CASE("Node transformations with sequential set_parent") {
    RssManager rssMgr;
    ...
    lvl1ch1->set_parent(root);
    lvl1ch2->set_parent(root);

    lvl2ch1->set_parent(lvl1ch1);
...
TEST_CASE("Node transformations with non-sequential set_parent") {
    RssManager rssMgr;
    FactoryManager factoryMgr(rssMgr);
